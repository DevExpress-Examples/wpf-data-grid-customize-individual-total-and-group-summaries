...
Namespace DXSample
    ...
    Public Class ItemsControlEx
        ...
        Private Sub Subscribe(ByVal collection As ObservableCollection(Of GridGroupSummaryColumnData))
            If collection IsNot Nothing Then AddHandler collection.CollectionChanged, AddressOf OnCollectionChanged
        End Sub

        Private Sub UnSubscribe(ByVal collection As ObservableCollection(Of GridGroupSummaryColumnData))
            If collection IsNot Nothing Then RemoveHandler collection.CollectionChanged, AddressOf OnCollectionChanged
        End Sub

        Private Sub OnCollectionChanged(ByVal sender As Object, ByVal e As Collections.Specialized.NotifyCollectionChangedEventArgs)
            UpdateItemsSource()
        End Sub

        Private Sub UpdateItemsSource()
            Dim data As List(Of GridGroupSummaryData) = New List(Of GridGroupSummaryData)()
            If Column IsNot Nothing AndAlso AllItems IsNot Nothing Then
                For Each item In AllItems
                    If item.Column Is Column Then data.Add(item)
                Next
            End If
